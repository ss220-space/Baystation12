...
/decl/emote/visible/dance/do_extra(var/atom/user)
	var/matrix/m15 = matrix(user.transform)
	m15.Turn(15)
	var/matrix/m345 = matrix(user.transform)
	m345.Turn(345)
	var/init_loc = user.loc
	animate(user, transform = m15, pixel_x = rand(-5, 5), pixel_y = rand(-5, 5), time = 1, 3)
