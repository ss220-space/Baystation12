...
/decl/emote/visible/dance/do_extra(var/atom/user)
	var/chosen_dance = rand(1, 2)
	switch(chosen_dance)
		if(1)
			user.SpinAnimation(15,1)
			user.set_dir(pick(GLOB.cardinal))
		if(2)
			var/matrix/initial_matrix = matrix(user.transform)
			user.SpinAnimation(7,1)
			user.set_dir(pick(GLOB.cardinal))
			sleep(1)
			initial_matrix = matrix(user.transform)
			initial_matrix.Translate(0,1)
			animate(user, transform = initial_matrix, time = 1, loop = 3)
			initial_matrix = matrix(user.transform)
//			initial_matrix.Translate(0,1)
//			animate(user, transform = initial_matrix, time = 1, loop = 0)
//			initial_matrix = matrix(user.transform)
//			initial_matrix.Translate(0,1)
//			animate(user, transform = initial_matrix, time = 1, loop = 0)
//			initial_matrix = matrix(user.transform)
...
//			animate(user, transform = initial_matrix, time = 1, loop = 0)
			...
			switch(user.dir)
				if(NORTH)
					...
					animate(user, transform = initial_matrix, time = 1, loop = 2)
				if(SOUTH)
					initial_matrix = matrix(user.transform)
					initial_matrix.Translate(0,-3)
					animate(user, transform = initial_matrix, time = 1, loop = 2)
