...
/decl/hierarchy/skill/organizational/bureaucracy
	...
	levels = list( 	"Необученный"	= "Обычно вы можете заполнить основные документы, хотя и с некоторыми ошибками. Вы имеете смутное представление о законе, почерпнутое в основном из новостей и личного опыта.",
					...
					"Мастер"		= "Вы можете заставить бумагу плясать под вашу дудку и с легкостью и знанием дела ориентироваться в самых запутанных бюрократических структурах. Ваши отчеты - это литературные произведения. Ваши знания закона обширны и глубоки, и вы можете быть сертифицированы для юридической практики.<br>- Вы можете говорить на языке, состоящем из запутанных бюрократических терминов и определений, который без глубоких знаний в области бюрократии почти невозможно понять.")
...
/decl/hierarchy/skill/organizational/bureaucracy/update_special_effects(mob/mob, level)
	...
	if(level == SKILL_PROF)
		if(ishuman(mob))
			...
			if(!(H.species.name in RESTRICTED_SPECIES))
