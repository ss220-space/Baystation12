...
/datum/antagonist/ert/Initialize()
	..()
	leader_welcome_text = SPAN_BOLD("Вы являетесь лидером Отряда Чрезвычайного Реагирования. ") + "Как лидер, вы отвечаете только перед должностными лицами [GLOB.using_map.company_name]. Вы уполномочены переназначать главу отдела там, где это необходимо для достижения ваших целей. Тем не менее, рекомендуется, чтобы вы работали с первоначальными главами для достижения своих целей, если это возможно."
	welcome_text =        SPAN_BOLD("Вы член Отряда Чрезвычайного Реагирования.") + "Как член Отряда Чрезвычайного Реагирования Вы отвечаете только перед своим лидером и должностными лицами [GLOB.using_map.company_name]."
