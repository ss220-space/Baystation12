/datum/job/submap/medical_ship_pilot
	title = "Rescue Pilot"
	total_positions = 1
	outfit_type = /decl/hierarchy/outfit/job/medical_ship_pilot
...
/datum/job/submap/medical_ship_technician
	...
	total_positions = 1
	outfit_type = /decl/hierarchy/outfit/job/medical_ship_technician
	info = "Ваш корабль получил катастрофические повреждения, что перебили свет и обрекли его на медленную смерть от астероидов \
...
/datum/job/submap/medical_ship_medic
	...
	supervisors = "Капитаном"
	total_positions = 2
	outfit_type = /decl/hierarchy/outfit/job/medical_ship_medic
	info = "Ваш корабль получил катастрофические повреждения, что перебили свет и обрекли его на медленную смерть от астероидов \
	в глубинах неисследованного космоса. Работайте сообща с Первым Помощником и оставшимся экипажем, и возможно, \
	вам удастся продержаться достаточно долго, чтобы спастись или даже вернуться в порт."
	required_language = LANGUAGE_SPACER
...
/datum/job/submap/medical_ship_pilot/equip(var/mob/living/carbon/human/H)
	setup_away_account(H)
	return ..()

/datum/job/submap/medical_ship_technician/equip(var/mob/living/carbon/human/H)
	setup_away_account(H)
	return ..()

/datum/job/submap/medical_ship_medic/equip(var/mob/living/carbon/human/H)
	setup_away_account(H)
	return ..()
