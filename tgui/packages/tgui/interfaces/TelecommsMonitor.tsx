      ...
      <Stack.Item>
        <Section
          ...
          <LabeledList>
            ...
            <LabeledList.Item label="Network Entity">{machine.name}</LabeledList.Item>
          </LabeledList>
        </Section>
      </Stack.Item>
      <Stack.Item grow>
          ...
          <Table>
            ...
            {linked_machinery?.map((lm) => (
              <Table.Row key={lm.ref} className="candystripe">
                ...
                <Table.Cell>
                  <Button content={lm.name} onClick={() => act('view_machine', { id: lm.id })} />
                </Table.Cell>
              </Table.Row>
            ))}
          </Table>
    ...
    <Stack fill vertical>
      <Stack.Item>
        <Section>
          ...
          <Button content="Probe Network" onClick={() => act('probe_network', { network_id: networkId })} />
        </Section>
      </Stack.Item>
      <Stack.Item grow>
        <Section
          ...
          <Table>
            ...
            {machinery?.map((machine) => (
              <Table.Row key={machine.ref} className="candystripe">
                ...
                <Table.Cell>
                  <Button content={machine.name} onClick={() => act('view_machine', { id: machine.id })} />
                </Table.Cell>
              </Table.Row>
            ))}
          </Table>
...
export const TelecommsMonitor = (props, context) => {
  ...
  return (
    <Window width={575} height={400}>
      <Window.Content>
        <Stack vertical fill>
          <Stack.Item>{!!error_message && <NoticeBox>{error_message}</NoticeBox>}</Stack.Item>
          <Stack.Item grow>
            {(screen === Screen.Main && <MainScreen />) || (screen === Screen.Machine && <MachineScreen />)}
          </Stack.Item>
        </Stack>
      </Window.Content>
    </Window>
